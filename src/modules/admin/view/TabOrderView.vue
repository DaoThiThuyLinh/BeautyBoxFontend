        ...
        <div v-for="item in orders" :key="item.id">
          <BaseCollapse :is-show-divider-header="true">
            ...
            <div class="space-y-4">
              ...
              <!-- <template v-if="tabActive === 'WAITING_FOR_PAYMENT'">
                <p class="text-right text-sm underline" @click="handlePayment(item.id)">Thanh toán</p>
              </template> -->
            </div>
          </BaseCollapse>
        </div>
...
<script setup lang="ts">
  ...
  import type { IOrder, IParamsOrder } from '@/types/order.types'
